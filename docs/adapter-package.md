...
> 适配器（adapter）是 OTT 生态中可独立分发、校验、签名的源单元。v1 定义包布局与 `adapter.json` 字段；运行时消费与 CI 流水线见 typetype ADR-011 Phase 2.2/2.4。权威 schema 见 `schemas/ott-adapter-v1.schema.json`。
