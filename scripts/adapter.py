...
SCHEMA_PATH = ROOT / "schemas" / "ott-adapter-v1.schema.json"

# 协议仓内置的客户端 API 能力常量（与 typetype CLIENT_API_LEVEL 保持同步）。
# validate 的 rights 步骤用它做能力对照；实际运行时的能力判定在客户端侧，
# 协议仓 SDK 不 import 任何客户端实现（三仓职责分离）。
CLIENT_API_LEVEL = 2
...
def _load_schema() -> dict:
    ...
    if not SCHEMA_PATH.exists():
        raise SystemExit(f"未找到权威 schema: {SCHEMA_PATH}")
...
def validate_package(package_dir: Path) -> list[tuple[str, bool, str]]:
    ...
    # (5) rights.min_api_level 对照 CLIENT_API_LEVEL（协议仓本地常量）
...
def run_debug(package_dir: Path, max_pages: int) -> int:
    ...
    协议仓 SDK 是纯校验工具：载荷的实际执行（rule 解释器 / script 沙箱）
    是客户端（typetype）侧职责，协议仓不 import 客户端实现。本命令对
    rule/script/instance 做离线静态评估；实际抓取请在客户端中订阅验证。
    """
    try:
    ...
    if type_ == "rule":
        ...
        print(f"steps: {'有（L1.5 DSL）' if rule.get('steps') else '无'}")
        print(f"pagination: {rule.get('pagination', {})}")
        print("-" * 60)
        print("静态评估通过。实际抓取执行请在客户端（typetype）中订阅验证。")
        return 0

    if type_ == "script":
        print(f"script: {content.get('path', 'code/script.py')}")
        print(f"fixtures: {adapter.get('fixtures', 'fixtures')}")
        print("-" * 60)
        print("脚本实际执行走客户端沙箱（typetype ScriptSandbox）。")
        print("本地预演请使用客户端侧 SDK 或 scripts/debug_rule.py（离线 CLI）。")
        return 0
