...
| `title` | `string` | ✅ | 文本标题（始终为最新条目的标题） |
| `content` | `string` | ✅ | 正文内容（始终为最新条目的内容） |
| `metadata` | `object` | ❌ | 扩展元数据（始终为最新条目的元数据） |
| `entries` | `array` | ❌ | 历史条目列表，按时间正序排列（可选，v2 引入） |
...
|:---|:---|:---|:---|
| `title` | `string` | ✅ | 本条目的标题 |
| `content` | `string` | ✅ | 本条目的正文字 | 
| `metadata` | `object` | ❌ | 本条目的元数据 |
| `fetched_at` | `string` | ✅ | 抓取时间（ISO 8601，如 `2026-07-07T10:00:00+08:00`） |

> **注意**：`entries` 为可选字段。旧格式文件（无 `entries`）仍被支持，脚本首次运行时会自动将旧内容迁移为第一条历史记录。顶层 `content`/`title` 始终指向最新条目，确保向后兼容。客户端如果不需要历史版本，直接读取顶层字段即可。`charCount` 为最新条目正文字数。`entries_count` 反映历史条目总数。|
